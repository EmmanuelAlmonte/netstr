...
namespace Netstr.Tests
{
    public class RateLimitingTests
    {
        ...
        [Fact]
        public async Task EventRateLimitExemptPublicKeyBypassesLimitTest()
        {
            var e = this.GetValidEvent();
            this.factory.WhitelistOptions = new WhitelistOptions
            {
                RateLimitExemptPublicKeys = [e.PublicKey]
            };
            ...
            var replies = new List<JsonElement[]>();
            var tooManyCount = limits.Value.Events.MaxEventsPerMinute + 3;
            ...
            replies.Should().HaveCount(tooManyCount);
            replies.Select(x => x[2].GetBoolean()).Should().AllBeEquivalentTo(true);
        }

        [Fact]
        public async Task NonExemptPublicKeyIsStillRateLimitedWhenExemptionsConfiguredTest()
        {
            this.factory.WhitelistOptions = new WhitelistOptions
            {
                RateLimitExemptPublicKeys = ["ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"]
            };
